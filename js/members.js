...
const memberCall = document.getElementById("member-call");
const memberIdNumber = document.getElementById("member-id-number");
const memberAdmissionFee = document.getElementById("member-admission-fee");
...
function resetMemberForm() {
    ...
    if (memberCall) memberCall.value = "";
    if (memberIdNumber) memberIdNumber.value = "";
    if (memberAdmissionFee) memberAdmissionFee.value = "";
}
...
function fillMemberForm(data) {
    ...
    if (memberCall) memberCall.value = data.call || "";
    if (memberIdNumber) memberIdNumber.value = data.idNumber || "";
    if (memberAdmissionFee) memberAdmissionFee.value = data.admissionFee ?? "";
    isEditing = true;
...
function renderMembers(members) {
    ...
    <div class="member-card-header">
        ...
        <span class="member-card-plan ${planClass}">
            ${escapeHTML(member.plan || "No Plan")}
        </span>
    </div>
    ...
    <div class="member-card-details">
        ...
        <span class="member-card-detail">
            ...
            <span class="value">
                ${escapeHTML(member.duration || "N/A")}
            </span>
        </span>
        ...
        <span class="member-card-detail">
            ...
            <span class="value">
                ID: ${escapeHTML(member.idNumber || "N/A")}
            </span>
        </span>
        ...
        <span class="member-card-detail">
            ...
            <span class="value">
                Admission: ৳${escapeHTML(member.admissionFee ?? "0")}
            </span>
        </span>
    ...
    <div class="member-card-actions">
        <button
            class="btn btn-gold btn-edit-member"
            data-id="${member.id}"
...
if (searchTerm) {
    filtered = filtered.filter(m => {
        ...
        const idNumber = String(m.idNumber || "").toLowerCase();
...
memberForm.addEventListener("submit", async (e) => {
    ...
    const call = memberCall ? memberCall.value.trim() : "";
const idNumber = memberIdNumber ? memberIdNumber.value.trim() : "";
const admissionFee = memberAdmissionFee ? parseFloat(memberAdmissionFee.value) || 0 : 0;
