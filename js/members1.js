...
function renderMembers(members) {
    ...
    const sorted = [...members].sort((a, b) => {
        const da = a.createdAt && typeof a.createdAt.toDate === 'function'
    ? a.createdAt.toDate()
    : new Date(0);

const db = b.createdAt && typeof b.createdAt.toDate === 'function'
    ? b.createdAt.toDate()
    : new Date(0);
        return db - da;
    });
...
function createMemberCard(member) {
    ...
    priceDetail.innerHTML = `<i class="fas fa-money-bill-wave"></i> <span class="value">${escapeHTML(priceDisplay)}</span>`;
    ...
    // ID
    const idDetail = document.createElement('span');
    idDetail.className = 'member-detail';
    idDetail.innerHTML = `<i class="fas fa-id-card"></i> <span class="value">ID: ${escapeHTML(member.idNumber || 'N/A')}</span>`;
    details.appendChild(idDetail);

    // Admission Fee
    const admissionDetail = document.createElement('span');
    admissionDetail.className = 'member-detail';
    admissionDetail.innerHTML = `<i class="fas fa-receipt"></i> <span class="value">Admission: ৳${escapeHTML(member.admissionFee ?? "0")}</span>`;
    details.appendChild(admissionDetail);
